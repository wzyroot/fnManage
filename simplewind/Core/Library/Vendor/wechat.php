...
class Wechat
{
	...
	const CARD_MEMBERCARD_GET             = '/card/membercard/userinfo/get?';    //获取会员信息
	...
	public function valid($return=false)
    {
        ...
        if ($_SERVER['REQUEST_METHOD'] == "POST") {
        	...
        	//file_put_contents('1234.txt',$echoStr);
        	if ($return) {
	...
	*/
	...
	public function getRevCardGet(){
	    ...
	    if (isset($this->_receive['OuterStr'])) //领取场景值，用于领取渠道数据统计。
	        $array['OuterStr'] = $this->_receive['OuterStr'];
	    if (isset($array) && count($array) > 0) {
	        return $array;
	    } else {
	        return false;
...
<Nonce><![CDATA[%s]]></Nonce>
	...
	protected function getCacheAgain(){
		$re = $this->resetAuth($appid='');
		return $re;
	}
	...
	/**
	 * 获取JSAPI授权的卡券TICKET，与JSSDK中的jsticket不同
	 * @param string $appid 用于多个appid时使用,可空
	 * @param string $jsapi_ticket 手动指定jsapi_ticket，非必要情况不建议用
	 */
	public function getCardApiTicket($appid='',$cardapi_ticket=''){
		if (!$this->access_token && !$this->checkAuth()) return false;
		if (!$appid) $appid = $this->appid;
		if ($cardapi_ticket) { //手动指定token，优先使用
		    $this->cardapi_ticket = $cardapi_ticket;
		    return $this->cardapi_ticket;
		}
		$authname = 'wechat_cardapi_ticket'.$appid;
		if ($rs = $this->getCache($authname))  {
			$this->cardapi_ticket = $rs;
			return $rs;
		}
		...
		if ($result)
		{
			...
			$this->cardapi_ticket = $json['ticket'];
			$expire = $json['expires_in'] ? intval($json['expires_in'])-200 : 7000;
			$this->setCache($authname,$this->cardapi_ticket,$expire);
			return $this->cardapi_ticket;
		}
		return false;
	}
...
*/		
	 ...
	 * @return array|bool 返回签名字串
	 */
	public function getCardSign($card_id,$timestamp, $noncestr){
		
	    if (!$this->cardapi_ticket && !$this->getCardApiTicket($appid)) return false;
		
	    $arrdata = array("api_ticket" => $this->cardapi_ticket,"timestamp" => $timestamp, "card_id" => $card_id, "noncestr" => $noncestr);
		$arr1  = array_values($arrdata);   
        sort( $arr1, SORT_STRING );
        $sign =  sha1(implode($arr1)) ;		
	    if (!$sign)
	        return false;
	    $CardSign = array(
	            "timestamp" => $timestamp,
	...
	*/
	public function getTags() {
		if (!$this->access_token && !$this->checkAuth()) return false;
        		$result = $this->http_get(self::API_URL_PREFIX.self::GET_TAGS.'access_token='.$this->access_token);
        		if ($result) {
			$json = json_decode($result,true);
			if (!$json || !empty($json['errcode'])) {
				$this->errCode = $json['errcode'];
	...
	public function getUserInfo($openid){
		...
		$result = $this->http_get(self::API_URL_PREFIX.self::USER_INFO_URL.'access_token='.$this->access_token.'&openid='.$openid.'&lang=zh_CN');
		if ($result)
		{	
			$json = json_decode($result,true);
			if (isset($json['errcode'])) {
	...
	public function sendTemplateMessage($data){
		...
		if (empty($this->access_token)){
			...
			$newdata = urldecode(json_encode($data));
			$result = $this->http_post(self::API_URL_PREFIX.self::TEMPLATE_SEND_URL.'access_token='.$this->access_token,$newdata);
			if($result){
				//print_r($result."<br>");
				$json = json_decode($result,true);
				if (!$json || !empty($json['errcode'])) {
    ...
    /**
     * 查询会员卡详情
     * @param string $card_id $code
     * @return boolean|array    返回数组信息比较复杂，请参看卡券接口文档
     */
    public function getVipCardInfo($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_MEMBERCARD_GET . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
            if (!$json || !empty($json['errcode'])) {
                $this->errCode = $json['errcode'];
     ...
     * @return boolean|string
     */
    public function createCardQrcode($card_id,$code='',$openid='',$expire_seconds=0,$is_unique_code=false,$balance='',$outer_str='') {
        $card = array(
                'card_id' => $card_id
        );
        if ($code)
    ...
    public function createCardQrcode1($data) {
        if (!$this->access_token && !$this->checkAuth()) return false;
        $result = $this->http_post(self::API_BASE_URL_PREFIX . self::CARD_QRCODE_CREATE . 'access_token=' . $this->access_token, self::json_encode($data));
        if ($result) {
            $json = json_decode($result, true);
     ...
     * @copyright Show More
     */
    public function addShakeAroundPage($data){
        if (!$this->access_token && !$this->checkAuth()) return false;
