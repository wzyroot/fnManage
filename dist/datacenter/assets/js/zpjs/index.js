...
$(document).ready(function(){
	...
	//turnplate.colors = ["#FF8584", "#FFEE7B", "#FF8584", "#FFEE7B","#FF8584", "#FFEE7B","#000"];
	...
	$('.info_tj').click(function(){
		...
		$.ajax({
			type : 'post',
			url : 'index.php?g=Portal&m=Active&a=zjmd',
			data : {
				hdid : hdid,
				jpid : jpid,
				name : name,
	...
	$('#tupBtn').click(function (){
		...
		if (hdtime==1) {
			...
			$.ajax({
				type : 'post',
				url : 'index.php?g=Portal&m=Active&a=jp',
				data : {
					hdid : hdid,
					numtype : numtype
				},
				success : function(response){
					if (response=='fail') {
						...
						turnplate.bRotate = !turnplate.bRotate;
						rotateFn(response[3], turnplate.restaraunts[response[3]-1],response[2]);
					}
				}
			});		
		}
